...
class _ProgressScreenState extends State<ProgressScreen>
  ...
  List<ProgressGoal> get _filteredActive {
    if (_typeFilter == 'lecture') {
      return _activeGoals.where((g) => g.goalType == 'lecture').toList();
    }
    if (_typeFilter == 'textbook') {
      return _activeGoals.where((g) => g.goalType == 'textbook').toList();
    }
    return _activeGoals;
  }
        ...
        ...lectures.map((goal) {
          ...
          return Container(
            ...
            decoration: BoxDecoration(
              gradient: isNext
                ? LinearGradient(
                    colors: [color.withOpacity(isDark ? 0.15 : 0.08), color.withOpacity(isDark ? 0.05 : 0.02)])
                : null,
              color: isNext ? null : (isDark ? Colors.white.withOpacity(0.02) : Colors.grey.shade50),
            ...
            child: Row(children: [
              SizedBox(width: 32, height: 32,
                child: Stack(alignment: Alignment.center, children: [
                  CircularProgressIndicator(
                    value: goal.progressPercent / 100, strokeWidth: 2.5,
                    backgroundColor: color.withOpacity(0.15),
                    valueColor: AlwaysStoppedAnimation(color)),
                  Text('${goal.progressPercent.round()}%', style: TextStyle(
              ...
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Text('${goal.subject} · ${next}$unitLabel부터 · ${goal.currentUnit}/${goal.totalUnits}$unitLabel',
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [color.withOpacity(isDark ? 0.15 : 0.08), color.withOpacity(isDark ? 0.05 : 0.03)]),
      ...
      child: Row(children: [
        ...
        Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Text(goal.title, style: TextStyle(
            ...
            color: isDark ? Colors.white : const Color(0xFF1E293B)),
            maxLines: 1, overflow: TextOverflow.ellipsis),
          Text('${next}$unitLabel부터 · ${goal.currentUnit}/${goal.totalUnits}$unitLabel 완료',
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
              ...
              child: CustomScrollView(
                slivers: [
                  ...
                  if (_allLectureGoals.isNotEmpty)
                    SliverToBoxAdapter(child: _buildAllLecturesList(isDark)),
                  // ─── 과목별 요약 ───
                  if (_activeGoals.isNotEmpty)
                    SliverToBoxAdapter(child: _buildSubjectSummary(isDark)),
                  // ─── 타입 필터 ───
                  ...
                  if (_completedGoals.isNotEmpty)
                    SliverToBoxAdapter(child: _buildCompletedSection(isDark)),
      ...
      floatingActionButton: FloatingActionButton.extended(
        ...
        label: const Text('목표 추가', style: TextStyle(fontWeight: FontWeight.w700)),
  ...
  Widget _buildHeader(bool isDark) {
    ...
    final avgPct = totalActive > 0
        ? (_activeGoals.fold<double>(0, (s, g) => s + g.progressPercent) / totalActive)
        : 0.0;
        ...
        child: Row(
          children: subjStats.entries.map((e) {
            ...
            return Container(
              ...
              padding: const EdgeInsets.all(14),
              decoration: BoxDecoration(
                color: Color(cfg.light),
                borderRadius: BorderRadius.circular(16),
                border: Border(left: BorderSide(color: Color(cfg.color), width: 3)),
              ),
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Row(children: [
                    ...
                    if (e.value.delayed > 0) ...[
                      ...
                      Text('⚠${e.value.delayed}', style: TextStyle(
                          fontSize: 10, fontWeight: FontWeight.w700,
                          color: Colors.red.shade600)),
                    ],
                  ]),
                  const SizedBox(height: 8),
                  Text('$avgPct%', style: TextStyle(
                      fontSize: 22, fontWeight: FontWeight.w800,
                      color: Color(cfg.color))),
                  Text('${e.value.count}개 진행 · ${e.value.completed}개 완료',
                      style: TextStyle(fontSize: 10, color: Colors.grey.shade500)),
  ...
  Widget _buildTypeFilter(bool isDark) {
    final lecCount = _activeGoals.where((g) => g.goalType == 'lecture').length;
    final bookCount = _activeGoals.where((g) => g.goalType == 'textbook').length;
    return Padding(
      padding: const EdgeInsets.symmetric(horizontal: 16, vertical: 4),
      child: Row(
        children: [
          _filterChip('전체 ${_activeGoals.length}', 'all', const Color(0xFF6366F1), isDark),
          const SizedBox(width: 8),
          _filterChip('🎬 강의 $lecCount', 'lecture', const Color(0xFF7C3AED), isDark),
          const SizedBox(width: 8),
          _filterChip('📖 교재 $bookCount', 'textbook', const Color(0xFF059669), isDark),
        ],
      ),
    );
  }

  Widget _filterChip(String label, String type, Color color, bool isDark) {
    ...
    return Expanded(
      child: GestureDetector(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            color: selected ? color : (isDark ? Colors.white10 : Colors.white),
            borderRadius: BorderRadius.circular(10),
            border: Border.all(
  ...
  List<Widget> _buildGroupedGoals(bool isDark) {
    ...
    for (final key in sortedKeys) {
      ...
      final groupName = hasGroup
          ? (goalsInGroup.first.groupName ?? '그룹')
          : null;
      final isExpanded = !hasGroup || _expandedGroups.contains(key);
          ...
          child: GestureDetector(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                ...
                boxShadow: [
                  BoxShadow(
                    color: const Color(0xFF6366F1).withOpacity(isDark ? 0.06 : 0.04),
              ...
              child: Row(children: [
                ...
                Expanded(child: Column(
                  ...
                  children: [
                    Text(groupName!, style: TextStyle(
                      ...
                      color: isDark ? Colors.white : const Color(0xFF1E293B))),
                    const SizedBox(height: 2),
                    Text('${goalsInGroup.length}개 · ${groupProgress.round()}%',
                      style: TextStyle(
                ...
                SizedBox(width: 48, height: 48,
                  child: Stack(alignment: Alignment.center, children: [
                    ...
                    Text('${groupProgress.round()}%', style: TextStyle(
      ...
      if (isExpanded) {
        slivers.add(SliverList(
          delegate: SliverChildBuilderDelegate(
            (ctx, i) => _buildGoalCard(goalsInGroup[i], isDark),
    ...
    for (final k in groups.keys) {
      if (k != null && !_expandedGroups.contains(k) && !_initialGroupsSet) {
  ...
  double _groupProgress(List<ProgressGoal> goals) {
    ...
    final total = goals.fold<double>(0, (s, g) => s + g.progressPercent);
  ...
  Widget _buildGoalCard(ProgressGoal goal, bool isDark) {
    ...
    final unitLabel = isTextbook ? 'p' : (goal.unitName.isNotEmpty ? goal.unitName : '강');
    final color = Color(cfg.color);
    ...
    return Container(
      ...
      child: Stack(children: [
        ...
        Container(
          ...
          child: Column(children: [
            ...
            Row(children: [
              ...
              const SizedBox(width: 6),
              Expanded(child: Text(goal.title, style: TextStyle(
              ...
              if (goal.isOverdue)
                Container(
                  ...
                  decoration: BoxDecoration(
                    color: Colors.red.shade50, borderRadius: BorderRadius.circular(3)),
                  child: Text('🔥+${goal.overdueDays}일',
                    style: TextStyle(fontSize: 9, fontWeight: FontWeight.w700, color: Colors.red.shade600)),
                ),
              _goalMenu(goal),
            ]),
            const SizedBox(height: 6),
            ...
            // 하단: 단위 + 날짜 + 액션
            Row(children: [
              Text('${goal.currentUnit}/${goal.totalUnits}$unitLabel',
                style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: Colors.grey.shade600)),
              if (goal.startDate != null && goal.endDate != null) ...[
                const SizedBox(width: 8),
                Text('${goal.startDate!.substring(5)}~${goal.endDate!.substring(5)}',
                  style: TextStyle(fontSize: 9, color: Colors.grey.shade500)),
              ],
              const Spacer(),
              _actionBtn('📝', color, () => _showLogModal(goal)),
              const SizedBox(width: 4),
              _actionBtn('🔧', Colors.grey.shade600, () => _showDirectEditModal(goal)),
        ...
        // 그룹 좌측 액센트 바
        if (hasGroup)
          Positioned(left: 0, top: 6, bottom: 6,
    ...
    return PopupMenuButton<String>(
      ...
      itemBuilder: (_) => [
        ...
        const PopupMenuItem(value: 'delete',
            child: Text('🗑️ 삭제', style: TextStyle(color: Colors.red))),
      ],
      onSelected: (v) async {
      ...
      builder: (ctx) => StatefulBuilder(builder: (ctx, setLocal) {
        return Container(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            ...
            Text('📁 그룹 지정', style: const TextStyle(
            ...
            Wrap(spacing: 8, runSpacing: 8, children: [
              ...
              ...existingGroups.entries.map((e) =>
                _groupChip(e.value, selectedId == e.key,
            ...
            SizedBox(width: double.infinity, child: ElevatedButton(
              onPressed: () async {
                ...
                goal.groupName = selectedId != null ? existingGroups[selectedId] : null;
  ...
  Widget _buildCompletedSection(bool isDark) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: Border(top: BorderSide(color: Colors.green.shade400, width: 3)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(children: [
            Text('✅ 완료된 학습', style: TextStyle(
                fontSize: 15, fontWeight: FontWeight.w700,
                color: isDark ? Colors.white : const Color(0xFF1E293B))),
            const SizedBox(width: 8),
            Text('${_completedGoals.length}개', style: TextStyle(
                fontSize: 12, color: Colors.grey.shade500, fontWeight: FontWeight.w600)),
          ]),
          const SizedBox(height: 12),
          ..._completedGoals.map((g) {
            final cfg = _cfgFor(g.subject);
            final rounds = g.completionHistory.length;
            return Container(
              margin: const EdgeInsets.only(bottom: 8),
              ...
              decoration: BoxDecoration(
                color: Color(cfg.light),
                borderRadius: BorderRadius.circular(12),
                border: Border.all(color: Color(cfg.color).withOpacity(0.2)),
              ),
              child: Row(
                children: [
                  ...
                  Container(
                    ...
                    child: Text('✓ ${rounds > 0 ? rounds : 1}회',
                        style: TextStyle(fontSize: 11, fontWeight: FontWeight.w700,
                            color: Colors.green.shade700)),
                  ),
                  const SizedBox(width: 8),
                  GestureDetector(
                    onTap: () => _restartGoal(g),
                    child: Container(
                      padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                      decoration: BoxDecoration(
                        color: Color(cfg.color).withOpacity(0.1),
                        borderRadius: BorderRadius.circular(6),
                      ),
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  ...
                  DropdownButtonFormField<String>(
                    ...
                    items: _subjectConfig.keys
                        .map((s) => DropdownMenuItem(value: s,
                            child: Text('${_subjectConfig[s]!.emoji} $s')))
                        .toList(),
                    onChanged: (v) => setModalState(() => subject = v ?? subject),
                  ),
                  const SizedBox(height: 12),
                  TextField(controller: titleCtrl, decoration: _inputDeco('교재/강의명')),
                  const SizedBox(height: 12),
                  // ── 그룹 선택 ──
                  Row(children: [
                    Text('📁 그룹', style: TextStyle(
                      fontSize: 13, fontWeight: FontWeight.w700,
                      color: Colors.grey.shade700)),
                    const SizedBox(width: 8),
                    Expanded(child: Wrap(spacing: 6, runSpacing: 6, children: [
                      _groupChip('없음', selectedGroupId == null,
                        () => setModalState(() {
                          selectedGroupId = null;
                          selectedGroupName = null;
                        })),
                      ...existingGroups.entries.map((e) =>
                        _groupChip(e.value, selectedGroupId == e.key,
                          () => setModalState(() {
                  ...
                  // 유형 토글
                  Row(children: [
                    _typeToggle('🎬 강의', goalType == 'lecture', () =>
                        setModalState(() => goalType = 'lecture')),
                    const SizedBox(width: 8),
                    _typeToggle('📖 교재', goalType == 'textbook', () =>
                        setModalState(() => goalType = 'textbook')),
                  ]),
                  const SizedBox(height: 12),
                  if (goalType == 'lecture')
                    ...
                    Row(children: [
                      Expanded(child: TextField(controller: startPageCtrl,
                          keyboardType: TextInputType.number,
                          decoration: _inputDeco('시작 페이지'))),
                      const SizedBox(width: 8),
                      Expanded(child: TextField(controller: endPageCtrl,
                          keyboardType: TextInputType.number,
                          decoration: _inputDeco('종료 페이지'))),
                    ]),
                  const SizedBox(height: 12),
                  Row(children: [
                    Expanded(child: TextField(controller: startDateCtrl,
                        decoration: _inputDeco('시작일'),
                        onTap: () => _pickDate(ctx, startDateCtrl))),
                    const SizedBox(width: 8),
                    Expanded(child: TextField(controller: endDateCtrl,
                        decoration: _inputDeco('종료일'),
                        onTap: () => _pickDate(ctx, endDateCtrl))),
                  ]),
                  const SizedBox(height: 16),
                  SizedBox(
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        decoration: BoxDecoration(
          ...
          borderRadius: BorderRadius.circular(8),
          border: Border.all(
            color: selected ? const Color(0xFF6366F1).withOpacity(0.4) : Colors.transparent),
        ),
        child: Text(label, style: TextStyle(
  ...
  Future<void> _saveNewGoal({
    ...
    if (goalType == 'lecture') {
      ...
      if (totalUnits <= 0) { _snack('총 강의 수를 입력하세요'); return; }
      unitName = '강';
    } else {
      sp = int.tryParse(startPage) ?? 0;
      ep = int.tryParse(endPage) ?? 0;
      if (ep <= sp) { _snack('종료 페이지가 시작보다 커야 합니다'); return; }
      totalUnits = ep - sp;
      unitName = 'p';
    }
      ...
      builder: (ctx) => Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 4),
            Text(goal.title, style: TextStyle(fontSize: 13, color: Colors.grey.shade600)),
            const SizedBox(height: 16),
            Row(children: [
              Expanded(child: TextField(controller: fromCtrl,
                  keyboardType: TextInputType.number,
                  decoration: _inputDeco(isTextbook ? '시작 페이지' : '시작 위치'))),
              Padding(padding: const EdgeInsets.symmetric(horizontal: 8),
                  child: Text('→', style: TextStyle(fontSize: 20))),
              Expanded(child: TextField(controller: toCtrl,
                  keyboardType: TextInputType.number,
                  decoration: _inputDeco(isTextbook ? '완료 페이지' : '종료 위치'))),
            ]),
            const SizedBox(height: 12),
            Row(children: [
              ...
              const SizedBox(width: 8),
              Expanded(child: TextField(controller: minsCtrl,
                  keyboardType: TextInputType.number,
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: const Text('📝 기록 저장', style: TextStyle(fontWeight: FontWeight.w700)),
              ),
            ),
          ],
        ),
  ...
  Future<void> _saveLog(ProgressGoal goal,
      {required int from, required int to, required int hours, required int mins}) async {
    if (to <= from) { _snack('종료값이 시작보다 커야 합니다'); return; }
    final isTextbook = goal.goalType == 'textbook';
    ...
    if (isTextbook) {
      ...
      if (to > goal.totalUnits) { _snack('최대 ${goal.totalUnits}까지입니다'); return; }
      goal.currentUnit = to;
    }
    ...
    goal.lastLogDate = DateFormat('yyyy-MM-dd').format(DateTime.now());
  ...
  void _showDirectEditModal(ProgressGoal goal) {
    final ctrl = TextEditingController(text: '${goal.currentUnit}');
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (ctx) => Container(
        ...
        child: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            Text('🔧 진척도 직접 수정', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800)),
            const SizedBox(height: 8),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                onPressed: () async {
                  ...
                  Navigator.pop(context);
                  _safeSetState(() {});
                  _snack('진행도 ${goal.progressPercent.round()}%로 수정');
                },
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF6366F1),
                  foregroundColor: Colors.white,
  ...
  void _showEditGoalModal(ProgressGoal goal) {
    ...
    final totalCtrl = TextEditingController(text: '${goal.totalUnits}');
    final startDateCtrl = TextEditingController(text: goal.startDate ?? '');
    final endDateCtrl = TextEditingController(text: goal.endDate ?? '');
    final currentCtrl = TextEditingController(text: '${goal.currentUnit}');
          ...
          child: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 12),
                TextField(controller: titleCtrl, decoration: _inputDeco('제목')),
                const SizedBox(height: 12),
                TextField(controller: totalCtrl,
                    keyboardType: TextInputType.number,
  ...
  final tu = int.tryParse(totalCtrl.text);
  final newCur = int.tryParse(currentCtrl.text);
  ...
  if (tu != null && tu > 0) {
    ...
    final cur = (m['currentUnit'] as num?)?.toInt() ?? 0;
    if (cur > tu) m['currentUnit'] = tu;
  }

  // ✅ 날짜
  if (startDateCtrl.text.isNotEmpty) {
    m['startDate'] = startDateCtrl.text;
  } else {
    m.remove('startDate');
  }
  ...
  // ✅ currentUnit
  if (newCur != null) {
    final maxTu = (m['totalUnits'] as num?)?.toInt() ?? goal.totalUnits;
    m['currentUnit'] = newCur.clamp(0, maxTu);
  }

  _goals[idx] = ProgressGoal.fromMap(m);
  ...
  void _autoComplete(ProgressGoal goal) {
    ...
    final totalMin = goal.dailyLogs.fold<int>(0, (s, l) => s + (l.studyMinutes ?? 0));
  ...
  Future<void> _completeGoal(ProgressGoal goal) async {
    final ok = await _confirm('이 학습 목표를 완료 처리하시겠습니까?');
    if (!ok) return;
    _autoComplete(goal);
    await _fb.saveProgressGoals(_goals);
    _safeSetState(() {});
  ...
  Future<void> _restartGoal(ProgressGoal goal) async {
    final ok = await _confirm('"${goal.title}" 을(를) 다시 시작하시겠습니까?');
    if (!ok) return;
    goal.completed = false;
    goal.completedAt = null;
    goal.currentUnit = 0;
    goal.dailyLogs = [];
    goal.startDate = DateFormat('yyyy-MM-dd').format(DateTime.now());
    goal.endDate = null;
    await _fb.saveProgressGoals(_goals);
    _safeSetState(() {});
    final round = goal.completionHistory.length + 1;
  ...
  Future<void> _deleteGoal(ProgressGoal goal) async {
    final ok = await _confirm('정말 삭제하시겠습니까? 기록이 모두 사라집니다.');
    if (!ok) return;
